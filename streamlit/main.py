...
with st.expander("展開以查看數據預處理"):
    ...
    if st.button("執行數據預處理"):
        ...
        for i, column in tqdm(enumerate(st.session_state.df.columns), desc="執行數據預處理", total=len(st.session_state.df.columns), ncols=100, bar_format="{l_bar}{bar} [時間剩餘: {remaining}] @data_lemak"):
...
with st.expander("展開以查看特徵工程"):
    ...
    if st.button("執行特徵工程"):
        progress_text = st.empty()
        progress_bar = st.progress(0)
        for i, feature in tqdm(enumerate(features_to_process), desc="執行特徵工程", total=len(features_to_process), ncols=100, bar_format="{l_bar}{bar} [時間剩餘: {remaining}] @data_lemak"):
...
if tuning_method == "手動調整":
    ...
    if st.button("訓練模型"):
        ...
        with tqdm(total=n_splits, desc="總體進度") as pbar_outer:
            for fold, (train_index, val_index) in enumerate(kf.split(X_train_scaled)):
                ...
                progress_text.text(f"總體進度: {progress*100:.0f}% (Fold {fold+1}/{n_splits}) by @data_lemak")

        mean_cv_score = np.mean(cv_scores)
        with tqdm(total=100, desc="最終模型訓練  by @data_lemak") as pbar_final:
    ...
    if st.button("訓練模型"):
        ...
        with tqdm(total=n_iter, desc="超參數搜索進度  by @data_lemak") as pbar_search:
...
if 'model' in st.session_state:
    model_description = {
        "Random Forest": "隨機森林是一種集成學習方法，通過構建多棵決策樹並將其結果進行平均來提高預測準確性和控制過擬合。",
        "XGBoost": "XGBoost 是一種強大的梯度提升算法，通過逐步添加模型來最小化損失函數，並且具有高效的計算性能。",
        "LightGBM": "LightGBM 是一種基於樹的學習算法，專為高效處理大規模數據而設計，具有更快的訓練速度和更低的內存使用。"
    }
    st.markdown(f"#### 模型類型")
    st.markdown(f"```{model_option}```", unsafe_allow_html=True)
    st.markdown(f"```**{model_description.get(model_option, '未知模型')}**```", unsafe_allow_html=True)
    ...
    with col1:
        st.subheader("訓練時間")
        st.write(f"{st.session_state['training_time']:.2f} 秒")
    with col2:
        st.subheader("資料筆數")
        st.write(f"{st.session_state['num_samples']} 筆")
