...
from pages.ml_reference.lib.utils.mapping import dayname_to_daynumber
...
def _autodetect_dimensions(df: pd.DataFrame) -> List[Any]:
    ...
    for col in eligible_cols:
        ...
        if ((col == 'store_name') | (col == 'item_category') | (col == 'item_name')):
            detected_cols.append(col)
        elif (len(values) > 1) & (len(values) < 0.05 * len(df)):
            if(max(values) / min(values) < 500):
                if((col != 'big_promotion') & (col != 'propose_promotion') & (col != 'sales_promotion') & (col != 'small_promotion') & (col != 'anniversary_promotion') & (col != 'peak_promotion') & (col != 'range_promotion') & (col != 'first_day_anniversary_promotion')
                   & (col != 'first_segment_anniversary') & (col != 'middle_segment_anniversary') & (col != 'last_segment_anniversary')):
                    detected_cols.append(col)
